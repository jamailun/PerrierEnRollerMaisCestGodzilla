    ...
    class NavMeshBuilder2d {
        ...
        public static void CollectSources(GameObject root, List<NavMeshBuildSource> sources, NavMeshBuilder2dState builder) {
            foreach (var modifier in root.GetComponentsInChildren<NavMeshModifier>()) {
                //Debug.Log("navmeshmodfier, in " + modifier.gameObject.name);
        ...
        private static void AddDefaultWalkableTilemap(List<NavMeshBuildSource> sources, NavMeshBuilder2dState builder, NavMeshModifier modifier) {
            foreach(var tilemap in modifier.GetComponentsInChildren<Tilemap>()) {


                //Debug.Log("coucou ici la tilemap : " + modifier.gameObject.name);
                ...
                if(tilemap.gameObject != modifier.gameObject && tilemap.gameObject.GetComponent<NavMeshModifier>() != null) {
                    // PAR jamailun
                    // cas où un navmeshmodifier en cache un autre, permettre aux tilemap d'exister et tout
                    var mod = tilemap.gameObject.GetComponent<NavMeshModifier>();
                    if(mod.overrideArea) {
                        box.area = mod.area;
                    }
                }

                //if(!builder.hideEditorLogs) Debug.Log("add tilemap data " + box);
                sources.Add(box);
            }
        }
        ...
        public static void CollectSources(List<NavMeshBuildSource> sources, SpriteRenderer spriteRenderer, int area, NavMeshBuilder2dState builder)
        {
            ...
            //if(!builder.hideEditorLogs) Debug.Log("source (a) +" + spriteRenderer.gameObject.name);
            sources.Add(src);
        ...
        { 
            ...
            //if(!builder.hideEditorLogs)  Debug.Log("source (b) +" + collider.gameObject.name);
            sources.Add(src);
        ...
        public static void CollectTileSources(List<NavMeshBuildSource> sources, Tilemap tilemap, int area, NavMeshBuilder2dState builder)
        {
            ...
            for (int i = bound.xMin; i < bound.xMax; i++)
            {
                for (int j = bound.yMin; j < bound.yMax; j++)
                {
                    ...
                    //if(!builder.hideEditorLogs) Debug.Log("source (c) +" + src);
                    sources.Add(src);
